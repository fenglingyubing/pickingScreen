...
- `O`：切换模式
- `P`：打开配置界面
- `K`：清除附近掉落物（玩家所在区块为中心，**2 区块半径**范围内的所有掉落物实体）

## 三种模式怎么用（核心概念）

配置里维护的是“**匹配规则列表**”（哪些物品算“匹配”），不同模式对“匹配/不匹配”的处理不同：

- 注意：规则列表与模式属于**服务器配置**，同一服务器内所有玩家共用；单人模式下不同存档/世界也默认共用同一份配置。
...
- `删除选中`：删除列表里当前选中的一条
- `应用`：将本地改动同步到服务器并保存配置（不点“应用”只是在本地暂存）
- `帮助`：返回首次使用说明界面

## 从背包快速选择物品（物品匹配 GUI）

在玩家背包界面（默认 `E`）右上角会出现一个 “筛” 按钮：

- 点击 “筛” 打开“物品匹配”界面
- 左侧点击背包中的物品图标，会把该物品加入右侧列表
- 右侧列表双击可删除条目
- 点击“应用到拾取筛”会把选择的物品合并进当前规则（自动去重），并同步到服务器
- 快捷键：在背包界面把鼠标悬停在物品图标上，按 `A` 可直接把该物品加入拾取筛规则并同步到服务器（首次进入世界若还没同步到规则快照，会先显示“同步中…”，稍后再按一次即可）
