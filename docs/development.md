...
- 入口：玩家背包（默认 `E`）右侧空白区域新增 “筛” 按钮（位于右侧中下部，避免遮挡其他模组按钮），打开 `PickupFilterMatcherScreen`。
- 风格：`PickupFilterMatcherScreen` 采用“容器/格子”布局：上方为匹配列表格子区，下方为玩家背包格子区；重新打开时会从服务端快照加载并显示当前已保存的物品规则。
- 保存策略：匹配界面添加/移除会**自动保存**；配置界面添加/删除也会自动保存（“存/应用”按钮仅作为手动同步兜底）。
- 顶部按钮：
  - `存/应`：保存并同步当前列表
  - `清`：清空“物品规则”（保留未在此界面展示的高级规则）
  - `配`：打开配置界面
  - `<`：返回背包
- 快捷键：背包界面鼠标悬停在物品图标上按 `A`，可直接把该物品合并进规则并同步（依赖客户端已拿到一次配置快照，否则会先发起同步请求）。
- 手工验证建议：
  - 启动开发客户端：`./gradlew runClient`
  - 进入世界 → 打开背包 → 点击 “筛” → 选择若干物品 → 点 `应` 保存 → 再按 `P` 打开配置确认规则已追加并去重
