...
- 入口：玩家背包（默认 `E`）右侧按钮列底部（在其他模组按钮的下方）新增 “筛” 按钮，打开 `PickupFilterMatcherScreen`。
- 操作：左侧点击背包格子将物品加入右侧“匹配列表”；右侧双击删除；点击“应用到拾取筛”会把选择合并进当前规则并发包同步到服务器。
- 快捷键：背包界面鼠标悬停在物品图标上按 `A`，可直接把该物品合并进规则并同步（依赖客户端已拿到一次配置快照，否则会先发起同步请求）。
- 手工验证建议：
  - 启动开发客户端：`./gradlew runClient`
  - 进入世界 → 打开背包 → 点击 “筛” → 选择若干物品 → “应用到拾取筛” → 再按 `P` 打开配置确认规则已追加并去重
  - 进入世界 → 打开背包 → 鼠标悬停物品 → 按 `A` → 再按 `P` 打开配置确认规则已追加并去重
  - 进入世界 → 按 `O` 切换模式 → 屏幕上方 actionbar 会提示当前模式名称（也可用 `P` 打开配置界面确认当前模式）
